...
class MILCellModelmerge(nn.Module):
    ...
    def forward(self, x, cellposes, masks):
        ...
        # cell_encoder is a frozen (requires_grad=False) distilled ResNet-18 used
        # purely as a feature extractor. We run it under torch.no_grad() so PyTorch
        # does NOT retain its intermediate activations for backward — this is the
        # single biggest memory saving for large-batch training, since the encoder
        # processes B*N_max cell images per forward. Gradients still flow correctly
        # into self.embedding and all downstream trainable layers.
        #
        # When encoder_chunk_size > 0, the flattened [B*N, C, H, W] is split into
        # chunks to cap peak GPU memory (conv1 output [chunk, 64, 28, 28] in FP32
        # can reach 10+ GiB at large batch). Each chunk is independently forwarded
        # and the outputs are concatenated.
        with torch.no_grad():
            flat_x = x.view(-1, C, H, W)
            if self.encoder_chunk_size > 0 and flat_x.size(0) > self.encoder_chunk_size:
                parts = []
                for start in range(0, flat_x.size(0), self.encoder_chunk_size):
                    end = min(start + self.encoder_chunk_size, flat_x.size(0))
                    parts.append(self.cell_encoder(flat_x[start:end]))
                cell_feats = torch.cat(parts, dim=0)                    # [B*N, 512]
            else:
                cell_feats = self.cell_encoder(flat_x)                  # [B*N, 512]
        ...
        # the self-attention scores scale as O(B * N_max^2), which dominates
        # memory at large batch / many cells. Only enabled in training.
        if self.use_gradient_checkpointing and self.training:
            tr_out = cp.checkpoint(
                self.shared_transformer, gat_out,
                src_key_padding_mask=pad_mask, use_reentrant=False)
        else:
