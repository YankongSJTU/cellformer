...
class BatchDatset:
    ...
    def _read_images(self):
        ...
        self.filename=[filename['image'] for filename in tqdm(self.files)]

        self.__channels = False
        # self.annotations = np.array(
        #    [np.expand_dims(self._transform(filename['annotation']), axis=3) for filename in self.files])
#        self.annotations = np.array([np.expand_dims(self._transform( filename['annotation']), axis=2) for filename in tqdm(self.files)])
    ...
    def next_batch_test(self, batch_size):
        ...
        return self.images[start:end], self.annotations[start:end],self.filename[start:end]
